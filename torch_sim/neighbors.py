...
def wrapping_nl(  # noqa: PLR0915
    ...
    """Compute neighbor list for atomic systems, including periodic boundary conditions.

    This function constructs a neighbor list for atomic systems by finding all pairs
    of atoms within a given cutoff distance. For periodic systems, it includes
    interactions across periodic boundaries by considering periodic images of atoms.

    Key features:
    - Handles both periodic and non-periodic boundary conditions
    - Includes self-interactions across periodic boundaries
    - Returns both neighbor indices and corresponding periodic shift vectors
    - Optionally sorts neighbors by first index for better memory access patterns
    - Efficiently handles large systems using vectorized operations
    - TorchScript compatible for fast execution
    ...
    Args:
        ...
        sort_id: If True, sort the neighbor list by the first atom index for better memory
            access
    ...
    Returns:
        tuple containing:
            ...
            - shifts: Tensor of shape (num_neighbors, 3) containing the periodic
              ...
              neighbor pair.
              For non-periodic systems, these are zero vectors.

    Notes:
        - For periodic systems, the function considers periodic images by computing the
          minimum number of cell shifts needed based on the cutoff and cell dimensions
        - Self-interactions (i=j) are included only when they occur across periodic
          boundaries
        - The neighbor list includes both (i,j) and (j,i) pairs for complete force
          computation
        - Memory scales with system size and number of neighbors per atom
    """
    warnings.warn(
        "wrapping_nl has a bug when cells are not orthorhombic, "
        "use other neighbor list implementations",
        stacklevel=2,
    )
    ...
    i_neq_j = i_indices_final != j_indices_final
    mapping_sym = torch.stack((j_indices_final[i_neq_j], i_indices_final[i_neq_j]), dim=0)
