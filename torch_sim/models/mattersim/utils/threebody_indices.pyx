...
def compute_threebody(const int[:, ::1] bond_atom_indices,
    ...
    for idx in range(n_bond):
        n_bond_per_atom[bond_atom_indices[idx, 0]] += 1
    ...
    for idx in range(n_atom):
        n_triple_temp = n_bond_per_atom[idx] * (n_bond_per_atom[idx] - 1)
        for j in range(n_bond_per_atom[idx]):
            n_triple_ij[start + j] = n_bond_per_atom[idx] - 1
        n_triple += n_triple_temp
        n_triple_i[idx] = n_triple_temp
        start += n_bond_per_atom[idx]
    ...
    for idx in range(n_atom):
        for j in range(n_bond_per_atom[idx]):
            for k in range(n_bond_per_atom[idx]):
                if j != k:
